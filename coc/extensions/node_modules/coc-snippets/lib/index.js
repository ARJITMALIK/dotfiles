    ...
    for (let snip of snippets) {
      ...
      if (ultisnip) {
        item.data.ultisnip = {
          ...
          range: import_coc3.Range.create(position.line, character, position.line, character + snip.prefix.length),
...
var MassCodeProvider = class extends BaseProvider {
  ...
  async createSnippet(text) {
    ...
    const name = await import_coc6.window.requestInput("Snippet Name");
    if (!name) {
    ...
    if (this.mapItems().some((item) => item.prefix === name && filetypes.includes(item.filetype))) {
      import_coc6.window.showMessage(`Snippet with name ${name} for this filetype already exists`);
      return Promise.resolve();
    }
  ...
  return new Promise(function(resolve, reject) {
    ...
    req.on("error", function(err) {
      if (err.code === "ECONNREFUSED") {
        import_coc6.window.showMessage("massCode is not running");
      } else {
        reject(err);
      }
    });
...
var UltiSnippetsProvider = class extends BaseProvider {
  ...
  async init() {
    ...
    await import_coc7.workspace.nvim.command(`exe 'pyxfile '.fnameescape('${filepath}')`);
...
` + code + "\n";
  ...
  async checkContext(context) {
    ...
    return await import_coc7.workspace.nvim.call(`pyxeval`, "True if context else False");
  }
  async executePyCodes(lines) {
    try {
      await import_coc7.workspace.nvim.command(`pyx ${addPythonTryCatch(lines.join("\n"))}`);
    } catch (e) {
...
` + e.stack;
  ...
  async editSnippets(text) {
    ...
    let file = import_path4.default.join(snippetsDir, `${filetype}.snippets`);
  ...
  async executePythonCode(pythonCode) {
    try {
      ...
      await import_coc7.workspace.nvim.command(`exe 'pyxfile '.fnameescape('${tmpfile}')`);
    } catch (e) {
...
async function getSnippetsDirectory(configuration) {
  ...
  if (snippetsDir) {
    ...
    if (!import_path4.default.isAbsolute(snippetsDir)) {
      import_coc7.window.showMessage(`snippets.userSnippetsDirectory => ${snippetsDir} should be absolute path`, "warning");
      snippetsDir = null;
    }
  }
  if (!snippetsDir)
    snippetsDir = import_path4.default.join(import_path4.default.dirname(import_coc7.workspace.env.extensionRoot), "ultisnips");
...
async function insertSnippetEdit(edit) {
  ...
  await import_coc8.commands.executeCommand("editor.action.insertSnippet", import_coc8.TextEdit.replace(edit.range, edit.newText), option);
}
...
async function activate(context) {
  ...
  if (configuration.get("ultisnips.enable", true)) {
    ...
    subscriptions.push(import_coc8.commands.registerCommand("snippets.editSnippets", provider.editSnippets));
  }
  ...
  if (configuration.get("massCode.enable", false)) {
    ...
    subscriptions.push(import_coc8.commands.registerCommand("snippets.editMassCodeSnippets", provider.createSnippet.bind(provider)));
  }
  if (configuration.get("autoTrigger", true)) {
    import_coc8.events.on("TextInsert", async (bufnr, info) => {
      ...
      let position = import_coc8.Position.create(info.lnum - 1, info.pre.length);
  ...
  subscriptions.push(import_coc8.commands.registerCommand("snippets.openOutput", () => {
    void import_coc8.window.showOutputChannel("snippets", false);
  }));
